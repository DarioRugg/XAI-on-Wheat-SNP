...
myY  <- read.csv("/data/Pheno.csv", header = TRUE)
...
# load("myGM.csv")
myGM = read.csv("/data/myGM.csv", header = TRUE, row.names = 1)
dim(myGM)
myGD <- t(myGD)
...
write.csv(NAM_dat, file = "/data/NAM_dat(dataset).csv")

# This data contains filtered data, GD and GM
# save.image("/data/Filtered_Data")
